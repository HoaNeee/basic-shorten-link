...
export const login = async (account: string, password: string) => {
  try {
    ...
    if (user.provider === "google") {
      throw new ApiError(409, "Conflict, please login with google");
    }
...
export const register = async (
  ...
  password: string,
  username: string,
  fullname = "User " + generateString("number", 8)
) => {
  try {
...
export const saveUser = async (payload: Partial<TUser>) => {
  try {
    if (payload.password) {
      const { hashPass } = await hashPasswordUsingBcrypt(payload.password);
      payload.password = hashPass;
    ...
    const query = `INSERT INTO users (${columns.join(",")}) VALUES (${values
...
export const verifyAccountRegister = async (email: string, code: string) => {
  try {
    ...
    const [result] = await pool.query(
      `SELECT links.*, COUNT(logs_action.link_id) AS view FROM links
        ...
        WHERE user_id = ? AND status = ? AND deleted = ?
				GROUP BY (links.id)
        ORDER BY MIN(links.created_at) DESC
        LIMIT ? OFFSET ?`,
...
export const statisticsLinkWithUserId = async (user_id?: number) => {
  try {
    ...
    return {
      total_view: records.flat().length,
